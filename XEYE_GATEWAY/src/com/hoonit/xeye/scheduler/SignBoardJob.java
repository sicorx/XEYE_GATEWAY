					...
					if("1".equals(dataLoadComplete)){
						...
						if("0".equals(commStatus)){
							...
							if(nowHour >= 12 && nowMinute >= 0){
								...
								int nowMI = nowCal.get(Calendar.MINUTE);
								
								if(nowMI < 10){
									nowM = "0"+nowMI;
								}else{
									nowM = String.valueOf(nowMI);
								}
								
								int ssMI = ssCal.get(Calendar.MINUTE);
								
								if(ssMI < 10){
									ssM = "0"+ssMI;
								}else{
									ssM = String.valueOf(ssMI);
								}
								
								String nowS = String.valueOf(nowCal.get(Calendar.HOUR_OF_DAY)) + nowM;
								String ssS = String.valueOf(ssCal.get(Calendar.HOUR_OF_DAY)) + ssM;
								
								logger.info("nowS="+nowS);
								logger.info("ssS="+ssS);
								
								if(Integer.parseInt(nowS) >= Integer.parseInt(ssS)){
							...
							// 오전
								...
								if(Integer.parseInt(nowS) >= Integer.parseInt(srS)){
