		...
		if(temp == null){
			...
			if(responseFC == 3 || responseFC == 4){
				...
				if("40001".equals((String)tagMap.get("ADDRESS"))){
					...
					logger.info("CH1 전체 누적전력량(110)=" + ch1AccPwrAmtTotal);
				    resultMap.put(deviceOID + ".110.0", ch1AccPwrAmtTotal);
				    
				    // Ch1 R/S/T 순간전력량 SNMP 갱신
					logger.info("CH1 전체 순간전력량(111)=" + ch1PwrAmtTotal);
				    resultMap.put(deviceOID + ".111.0", ch1PwrAmtTotal);
				    
				    // Ch2 R/S/T 누적전력량 SNMP 갱신
					logger.info("CH2 전체 누적전력량(112)=" + ch2AccPwrAmtTotal);
				    resultMap.put(deviceOID + ".112.0", ch2AccPwrAmtTotal);
				    
				    // Ch2 R/S/T 순간전력량 SNMP 갱신
					logger.info("CH2 전체 순간전력량(113)=" + ch2PwrAmtTotal);
				    resultMap.put(deviceOID + ".113.0", ch2PwrAmtTotal);
				    
				    // Ch3 R/S/T 누적전력량 SNMP 갱신
					logger.info("CH3 전체 누적전력량(114)=" + ch3AccPwrAmtTotal);
				    resultMap.put(deviceOID + ".114.0", ch3AccPwrAmtTotal);
				    
				    // Ch3 R/S/T 순간전력량 SNMP 갱신
					logger.info("CH3 전체 순간전력량(115)=" + ch3PwrAmtTotal);
				    resultMap.put(deviceOID + ".115.0", ch3PwrAmtTotal);
				    
				    // Ch4 R/S/T 누적전력량 SNMP 갱신
					logger.info("CH4 전체 누적전력량(116)=" + ch4AccPwrAmtTotal);
				    resultMap.put(deviceOID + ".116.0", ch4AccPwrAmtTotal);
				    
				    // Ch4 R/S/T 순간전력량 SNMP 갱신
					logger.info("CH4 전체 순간전력량(117)=" + ch4PwrAmtTotal);
				    resultMap.put(deviceOID + ".117.0", ch4PwrAmtTotal);
				    
				    // Ch5 R/S/T 누적전력량 SNMP 갱신
					logger.info("CH5 전체 누적전력량(118)=" + ch5AccPwrAmtTotal);
				    resultMap.put(deviceOID + ".118.0", ch5AccPwrAmtTotal);
				    
				    // Ch5 R/S/T 순간전력량 SNMP 갱신
					logger.info("CH5 전체 순간전력량(119)=" + ch5PwrAmtTotal);
				    resultMap.put(deviceOID + ".119.0", ch5PwrAmtTotal);
				    
				    // Ch6 R/S/T 누적전력량 SNMP 갱신
					logger.info("CH6 전체 누적전력량(120)=" + ch6AccPwrAmtTotal);
				    resultMap.put(deviceOID + ".120.0", ch6AccPwrAmtTotal);
				    
				    // Ch6 R/S/T 순간전력량 SNMP 갱신
					logger.info("CH6 전체 순간전력량(121)=" + ch6PwrAmtTotal);
				    resultMap.put(deviceOID + ".121.0", ch6PwrAmtTotal);
