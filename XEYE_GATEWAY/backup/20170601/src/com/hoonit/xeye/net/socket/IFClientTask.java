...
public class IFClientTask extends TimerTask {
	...
	// 채널별 누적사용량
	private long ch1Acc  = 0L;
	private long ch2Acc  = 0L;
	private long ch3Acc  = 0L;
	private long ch4Acc  = 0L;
	private long ch5Acc  = 0L;
	private long ch6Acc  = 0L;
	private long ch7RAcc = 0L;
	private long ch7SAcc = 0L;
	private long ch7TAcc = 0L;
	private long ch8RAcc = 0L;
	private long ch8SAcc = 0L;
	private long ch8TAcc = 0L;
		...
		try{
			...
			if(!"".equals(storeCD)){
				...
				if("1".equals(dataLoadComplete)){
					...
					// 전체(총) 누적전력량
					File accPowerFile = new File("resource/conf/total_acc_power.txt");
					
					if(!accPowerFile.exists()){
						accPowerFile.createNewFile();
					}else{
						...
						try{
							...
							br = new BufferedReader(new FileReader(accPowerFile));
							...
							if(st.countTokens() == 13){
								...
								logger.info("Previous acc power save date="+pyear+"-"+pmonth+"-"+pdate+" "+phour+":"+pminute+":"+psecond);
									
								ch1Acc  = Long.parseLong(st.nextToken());
								ch2Acc  = Long.parseLong(st.nextToken());
								ch3Acc  = Long.parseLong(st.nextToken());
								ch4Acc  = Long.parseLong(st.nextToken());
								ch5Acc  = Long.parseLong(st.nextToken());
								ch6Acc  = Long.parseLong(st.nextToken());
								ch7RAcc = Long.parseLong(st.nextToken());
								ch7SAcc = Long.parseLong(st.nextToken());
								ch7TAcc = Long.parseLong(st.nextToken());
								ch8RAcc = Long.parseLong(st.nextToken());
								ch8SAcc = Long.parseLong(st.nextToken());
								ch8TAcc = Long.parseLong(st.nextToken());
			        ...
			        if("0".equals(commStatus)){
			        	...
			        	try{
			        		...
			        		// 현재누적사용량이 이전누적사용량보다 크거나 같으면
			        		// 총사용전력량 += 현재누적사용량 - 이전누적사용량
			        		if(Long.parseLong(dataMap.get(EnterpriseOIDManager.getEnterpriseOID() + ".10.1.0")) >= ch1PreAcc){
			        			ch1Acc += Long.parseLong(dataMap.get(EnterpriseOIDManager.getEnterpriseOID() + ".10.1.0")) - ch1PreAcc; 
			        		}
			        		// 현재누적사용량이 이전누적사용량보다 작으면 초기화(리셋) 됐다고 판단
			        		// 초기화 후 최초로 수집된 값을 더해준다.
			        		// 총사용전력량 += 현재누적사용량
			        		else{
			        			ch1Acc += Long.parseLong(dataMap.get(EnterpriseOIDManager.getEnterpriseOID() + ".10.1.0"));
			        		}
			        		...
			        		// 5분간 전력사용량
			        		use5M = ch1Acc - accTemp;
			        		
			        		temp = ByteUtils.toBytes(ch1Acc);
			        	}catch(Exception e){
			        	...
			        	try{
			        		...
			        		if(Long.parseLong(dataMap.get(EnterpriseOIDManager.getEnterpriseOID() + ".10.4.0")) >= ch2PreAcc){
			        			ch2Acc += Long.parseLong(dataMap.get(EnterpriseOIDManager.getEnterpriseOID() + ".10.4.0")) - ch2PreAcc; 
			        		}
			        		// 현재누적사용량이 이전누적사용량보다 작으면 초기화(리셋) 됐다고 판단
			        		// 초기화 후 최초로 수집된 값을 더해준다.
			        		// 총사용전력량 += 현재누적사용량
			        		else{
			        			ch2Acc += Long.parseLong(dataMap.get(EnterpriseOIDManager.getEnterpriseOID() + ".10.4.0"));
			        		}
			        		...
			        		// 5분간 전력사용량
			        		use5M = ch2Acc - accTemp;
			        		
			        		temp = ByteUtils.toBytes(ch2Acc);
			        	}catch(Exception e){
			        		temp = ByteUtils.toBytes(0L);
			        	}
			        	...
			        	try{
			        		...
			        		if(Long.parseLong(dataMap.get(EnterpriseOIDManager.getEnterpriseOID() + ".10.7.0")) >= ch3PreAcc){
			        			ch3Acc += Long.parseLong(dataMap.get(EnterpriseOIDManager.getEnterpriseOID() + ".10.7.0")) - ch3PreAcc; 
			        		}
			        		// 현재누적사용량이 이전누적사용량보다 작으면 초기화(리셋) 됐다고 판단
			        		// 초기화 후 최초로 수집된 값을 더해준다.
			        		// 총사용전력량 += 현재누적사용량
			        		else{
			        			ch3Acc += Long.parseLong(dataMap.get(EnterpriseOIDManager.getEnterpriseOID() + ".10.7.0"));
			        		}
			        		...
			        		// 5분간 전력사용량
			        		use5M = ch3Acc - accTemp;
			        		
			        		temp = ByteUtils.toBytes(ch3Acc);
			        	}catch(Exception e){
			        		temp = ByteUtils.toBytes(0L);
			        	}
			        	...
			        	try{
			        		...
			        		if(Long.parseLong(dataMap.get(EnterpriseOIDManager.getEnterpriseOID() + ".10.10.0")) >= ch4PreAcc){
			        			ch4Acc += Long.parseLong(dataMap.get(EnterpriseOIDManager.getEnterpriseOID() + ".10.10.0")) - ch4PreAcc; 
			        		}
			        		// 현재누적사용량이 이전누적사용량보다 작으면 초기화(리셋) 됐다고 판단
			        		// 초기화 후 최초로 수집된 값을 더해준다.
			        		// 총사용전력량 += 현재누적사용량
			        		else{
			        			ch4Acc += Long.parseLong(dataMap.get(EnterpriseOIDManager.getEnterpriseOID() + ".10.10.0"));
			        		}
			        		...
			        		// 5분간 전력사용량
			        		use5M = ch4Acc - accTemp;
			        		
			        		temp = ByteUtils.toBytes(ch4Acc);
			        	}catch(Exception e){
			        		temp = ByteUtils.toBytes(0L);
			        	}
			        	...
			        	try{
			        		...
			        		if(Long.parseLong(dataMap.get(EnterpriseOIDManager.getEnterpriseOID() + ".10.13.0")) >= ch5PreAcc){
			        			ch5Acc += Long.parseLong(dataMap.get(EnterpriseOIDManager.getEnterpriseOID() + ".10.13.0")) - ch5PreAcc; 
			        		}
			        		// 현재누적사용량이 이전누적사용량보다 작으면 초기화(리셋) 됐다고 판단
			        		// 초기화 후 최초로 수집된 값을 더해준다.
			        		// 총사용전력량 += 현재누적사용량
			        		else{
			        			ch5Acc += Long.parseLong(dataMap.get(EnterpriseOIDManager.getEnterpriseOID() + ".10.13.0"));
			        		}
			        		...
			        		// 5분간 전력사용량
			        		use5M = ch5Acc - accTemp;
			        		
			        		temp = ByteUtils.toBytes(ch5Acc);
			        	}catch(Exception e){
			        		temp = ByteUtils.toBytes(0L);
			        	}
			        	...
			        	try{
			        		...
			        		if(Long.parseLong(dataMap.get(EnterpriseOIDManager.getEnterpriseOID() + ".10.16.0")) >= ch6PreAcc){
			        			ch6Acc += Long.parseLong(dataMap.get(EnterpriseOIDManager.getEnterpriseOID() + ".10.16.0")) - ch6PreAcc; 
			        		}
			        		// 현재누적사용량이 이전누적사용량보다 작으면 초기화(리셋) 됐다고 판단
			        		// 초기화 후 최초로 수집된 값을 더해준다.
			        		// 총사용전력량 += 현재누적사용량
			        		else{
			        			ch6Acc += Long.parseLong(dataMap.get(EnterpriseOIDManager.getEnterpriseOID() + ".10.16.0"));
			        		}
			        		...
			        		// 5분간 전력사용량
			        		use5M = ch6Acc - accTemp;
			        		
			        		temp = ByteUtils.toBytes(ch6Acc);
			        	}catch(Exception e){
			        		temp = ByteUtils.toBytes(0L);
			        	}
			        	...
			        	try{
			        		...
			        		if(Long.parseLong(dataMap.get(EnterpriseOIDManager.getEnterpriseOID() + ".10.19.0")) >= ch7RPreAcc){
			        			ch7RAcc += Long.parseLong(dataMap.get(EnterpriseOIDManager.getEnterpriseOID() + ".10.19.0")) - ch7RPreAcc; 
			        		}
			        		// 현재누적사용량이 이전누적사용량보다 작으면 초기화(리셋) 됐다고 판단
			        		// 초기화 후 최초로 수집된 값을 더해준다.
			        		// 총사용전력량 += 현재누적사용량
			        		else{
			        			ch7RAcc += Long.parseLong(dataMap.get(EnterpriseOIDManager.getEnterpriseOID() + ".10.19.0"));
			        		}
			        		...
			        		// 5분간 전력사용량
			        		use5M = ch7RAcc - accTemp;
			        		
			        		temp = ByteUtils.toBytes(ch7RAcc);
			        	}catch(Exception e){
			        		temp = ByteUtils.toBytes(0L);
			        	}
			        	...
			        	try{
			        		...
			        		if(Long.parseLong(dataMap.get(EnterpriseOIDManager.getEnterpriseOID() + ".10.22.0")) >= ch7SPreAcc){
			        			ch7SAcc += Long.parseLong(dataMap.get(EnterpriseOIDManager.getEnterpriseOID() + ".10.22.0")) - ch7SPreAcc; 
			        		}
			        		// 현재누적사용량이 이전누적사용량보다 작으면 초기화(리셋) 됐다고 판단
			        		// 초기화 후 최초로 수집된 값을 더해준다.
			        		// 총사용전력량 += 현재누적사용량
			        		else{
			        			ch7SAcc += Long.parseLong(dataMap.get(EnterpriseOIDManager.getEnterpriseOID() + ".10.22.0"));
			        		}
			        		...
			        		// 5분간 전력사용량
			        		use5M = ch7SAcc - accTemp;
			        		
			        		temp = ByteUtils.toBytes(ch7SAcc);
			        	}catch(Exception e){
			        		temp = ByteUtils.toBytes(0L);
			        	}
			        	...
			        	try{
			        		...
			        		if(Long.parseLong(dataMap.get(EnterpriseOIDManager.getEnterpriseOID() + ".10.25.0")) >= ch7TPreAcc){
			        			ch7TAcc += Long.parseLong(dataMap.get(EnterpriseOIDManager.getEnterpriseOID() + ".10.25.0")) - ch7TPreAcc; 
			        		}
			        		// 현재누적사용량이 이전누적사용량보다 작으면 초기화(리셋) 됐다고 판단
			        		// 초기화 후 최초로 수집된 값을 더해준다.
			        		// 총사용전력량 += 현재누적사용량
			        		else{
			        			ch7TAcc += Long.parseLong(dataMap.get(EnterpriseOIDManager.getEnterpriseOID() + ".10.25.0"));
			        		}
			        		...
			        		// 5분간 전력사용량
			        		use5M = ch7TAcc - accTemp;
			        		
			        		temp = ByteUtils.toBytes(ch7TAcc);
			        	}catch(Exception e){
			        		temp = ByteUtils.toBytes(0L);
			        	}
			        	...
			        	try{
			        		...
			        		if(Long.parseLong(dataMap.get(EnterpriseOIDManager.getEnterpriseOID() + ".10.28.0")) >= ch8RPreAcc){
			        			ch8RAcc += Long.parseLong(dataMap.get(EnterpriseOIDManager.getEnterpriseOID() + ".10.28.0")) - ch8RPreAcc; 
			        		}
			        		// 현재누적사용량이 이전누적사용량보다 작으면 초기화(리셋) 됐다고 판단
			        		// 초기화 후 최초로 수집된 값을 더해준다.
			        		// 총사용전력량 += 현재누적사용량
			        		else{
			        			ch8RAcc += Long.parseLong(dataMap.get(EnterpriseOIDManager.getEnterpriseOID() + ".10.28.0"));
			        		}
			        		...
			        		// 5분간 전력사용량
			        		use5M = ch8RAcc - accTemp;
			        		
			        		temp = ByteUtils.toBytes(ch8RAcc);
			        	}catch(Exception e){
			        		temp = ByteUtils.toBytes(0L);
			        	}
			        	...
			        	try{
			        		...
			        		if(Long.parseLong(dataMap.get(EnterpriseOIDManager.getEnterpriseOID() + ".10.31.0")) >= ch8SPreAcc){
			        			ch8SAcc += Long.parseLong(dataMap.get(EnterpriseOIDManager.getEnterpriseOID() + ".10.31.0")) - ch8SPreAcc; 
			        		}
			        		// 현재누적사용량이 이전누적사용량보다 작으면 초기화(리셋) 됐다고 판단
			        		// 초기화 후 최초로 수집된 값을 더해준다.
			        		// 총사용전력량 += 현재누적사용량
			        		else{
			        			ch8SAcc += Long.parseLong(dataMap.get(EnterpriseOIDManager.getEnterpriseOID() + ".10.31.0"));
			        		}
			        		...
			        		// 5분간 전력사용량
			        		use5M = ch8SAcc - accTemp;
			        		
			        		temp = ByteUtils.toBytes(ch8SAcc);
			        	}catch(Exception e){
			        		temp = ByteUtils.toBytes(0L);
			        	}
			        	...
			        	try{
			        		...
			        		if(Long.parseLong(dataMap.get(EnterpriseOIDManager.getEnterpriseOID() + ".10.34.0")) >= ch8TPreAcc){
			        			ch8TAcc += Long.parseLong(dataMap.get(EnterpriseOIDManager.getEnterpriseOID() + ".10.34.0")) - ch8TPreAcc; 
			        		}
			        		// 현재누적사용량이 이전누적사용량보다 작으면 초기화(리셋) 됐다고 판단
			        		// 초기화 후 최초로 수집된 값을 더해준다.
			        		// 총사용전력량 += 현재누적사용량
			        		else{
			        			ch8TAcc += Long.parseLong(dataMap.get(EnterpriseOIDManager.getEnterpriseOID() + ".10.34.0"));
			        		}
			        		...
			        		// 5분간 전력사용량
			        		use5M = ch8TAcc - accTemp;
			        		
			        		temp = ByteUtils.toBytes(ch8TAcc);
			        	}catch(Exception e){
			        		temp = ByteUtils.toBytes(0L);
			        	}
			        ...
			        // 총사용량 저장
			        bw = new BufferedWriter(new FileWriter(accPowerFile));
			        ...
			        sb.append(",");
			        sb.append(String.valueOf(ch1Acc));
			        sb.append(",");
			        sb.append(String.valueOf(ch2Acc));
			        sb.append(",");
			        sb.append(String.valueOf(ch3Acc));
			        sb.append(",");
			        sb.append(String.valueOf(ch4Acc));
			        sb.append(",");
			        sb.append(String.valueOf(ch5Acc));
			        sb.append(",");
			        sb.append(String.valueOf(ch6Acc));
			        sb.append(",");
			        sb.append(String.valueOf(ch7RAcc));
			        sb.append(",");
			        sb.append(String.valueOf(ch7SAcc));
			        sb.append(",");
			        sb.append(String.valueOf(ch7TAcc));
			        sb.append(",");
			        sb.append(String.valueOf(ch8RAcc));
			        sb.append(",");
			        sb.append(String.valueOf(ch8SAcc));
			        sb.append(",");
			        sb.append(String.valueOf(ch8TAcc));
